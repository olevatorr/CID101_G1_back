...
export default {
    ...
    methods: {
        ...
        async fetchOrders() {
            try {
                const response = await axios.get('http://localhost/cid101/g1/api/productOrder.php');
                if (!response.data.error) {
                    this.productOrder = this.processOrders(response.data.productOrder);
                } else {
                    throw new Error(response.data.msg);
                }
            } catch (error) {
                console.error('獲取訂單數據時發生錯誤：', error);
            }
        },
        processOrders(orders) {
            ...
            orders.forEach(order => {
                if (!processedOrders[order.PO_ID]) {
                    processedOrders[order.PO_ID] = { ...order, details: [] };
                }
                processedOrders[order.PO_ID].details.push({
                    P_NAME: order.P_NAME,
    ...
    computed: {
        // 使用計算屬性來動態顯示付款方式和運送方式的中文
        paymentMethod() {
            switch (this.newItem.PM_ID) {
                case '0':
                    return '超商繳費';
                case '1':
                    return '銀行轉帳';
